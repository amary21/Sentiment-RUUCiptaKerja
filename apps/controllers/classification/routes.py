...
def analysis_process():
    ...
    _, x_tweet_test, _, _ = train_test_split(
        dataframe['tweet'], dataframe['sentimen'], test_size=0.2, shuffle=False)

    # split clean dataset for classification
    x_train, x_test, y_train, y_test = train_test_split(
        dataframe['clean_tweet'], dataframe['sentimen'], test_size=0.2, shuffle=False)
    # feature tf-idf
    tfidf = TfidfFeature()
    ...
    for i, row in enumerate(x_tweet_test):
        result = AnalysisResult(
            tweet=row, analysis_manual=y_test[i], analysis_system=predict[i])
